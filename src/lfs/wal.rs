...
impl LogFile {
    ...
    pub fn open<P: AsRef<std::path::Path>>(path: P) -> Result<LogFile, LogError> {
        ...
        let mut file = open_options.read(true).write(true).create(true).open(path.as_ref())?;
    ...
    pub fn compact(&mut self, new_start_index: u64) -> Result<(), LogError> {
        // Assuming flush() is adapted to the new std::fs::File too
        self.flush()?;
    
        // Seek to the start index.
        self.file.seek(SeekFrom::Start(new_start_index))?;
        ...
        new_file.write_all(&new_start_index.to_le_bytes())?;
        io::copy(&mut self.file, &mut new_file)?;
    
        std::fs::rename(&temp_file_path, &self.path)?;
        self.file = new_file;
