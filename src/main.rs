...
async fn main() {
    ...
    // kernel receives cards via this channel, all other modules send cards
    ...
    let tcp_listener = TcpListener::bind(format!("127.0.0.1:{}", our_port)).await.expect("Can't listen");
    ...
    let quit: String = tokio::select! {
        ...
        _ = websockets::ws_listener(print_sender.clone(), tcp_listener) => {
