    ...
    *networking_keypair_post.lock().unwrap() = Some(serialized_networking_keypair);
    ...
    let our = Identity {
        ...
        allowed_routers: if ip == "localhost" || !info.direct {
            vec![
                "testnode98.uq".into(), // "uqbar-router-1.uq".into(), // "0x8d9e54427c50660c6d4802f63edca86a9ca5fd6a78070c4635950e9d149ed441".into(),
...
pub async fn login(
    tx: mpsc::Sender<(String, Vec<String>, signature::Ed25519KeyPair, Vec<u8>)>,
    kill_rx: oneshot::Receiver<bool>,
    keyfile: Vec<u8>,
    port: u16,
) {
...
async fn handle_password(
    ...
    let (username, routers, networking_keypair, jwt_secret_bytes) =
